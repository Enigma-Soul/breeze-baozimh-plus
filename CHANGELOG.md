...
### Feat(去水印):
- wm1 版权横幅裁切：顶/底条带与模板匹配（带偏移容差，兼容略偏/略高如 202 的条带）；h>1200 时按内容高（h−1000）裁切
- 高度预判：仅 h>1005 的页全解码匹配，正常页直接跳过——规避 QuickJS-NG 沙箱里 jpeg 解码 ~5s/页的代价
- 纯白/损坏页与极小图（h<350）占位处理
- 纯 TS，无 WASM 依赖（沙箱无 WebAssembly）；算法对齐 Python 参考实现
...
- 调试日志：插件内部 fetch 自报到 dev server /log，不依赖宿主「调试日志地址」（设了那个会导致 bundle 加载失败）
